...
import { api, ContactPage } from "@/lib/api";
...
export default function ContactPage() {
  ...
  const data = contactData || {
    ...
    hero_description: 'Tell us about your home, project, or dream space. Our studio in Damauli, Tanahu will be in touch to arrange a thoughtful first conversation.',
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto grid max-w-7xl gap-16 px-6 pb-20 pt-32 md:grid-cols-[0.9fr_1.1fr] md:px-12 md:pb-28 md:pt-40">
        <div className="flex flex-col justify-between gap-14">
          <div>
            <p className="mb-6 text-xs uppercase tracking-[0.25em] text-muted-foreground">{data.hero_subtitle}</p>
            <h1 className="max-w-xl text-5xl font-light leading-[0.95] tracking-[-0.06em] md:text-8xl">{data.hero_title}</h1>
            <p className="mt-8 max-w-md text-base leading-7 text-muted-foreground">{data.hero_description}</p>
          </div>
          <div className="grid gap-6 border-t border-border pt-6 text-sm sm:grid-cols-2">
            <div>
              ...
              <p className="flex items-start gap-2 leading-6"><MapPin size={16} className="mt-1 shrink-0" /> {data.location_address}<br />{data.location_city}</p>
            </div>
            <div>
              <p className="mb-3 text-xs uppercase tracking-[0.2em] text-muted-foreground">Reach us</p>
              <a href={`tel:${data.phone_number}`} className="mb-2 flex items-center gap-2 hover:underline"><Phone size={15} /> {data.phone_number}</a>
              <a href={`mailto:${data.email_address}`} className="flex items-center gap-2 hover:underline"><Mail size={15} /> {data.email_address}</a>
            </div>
          </div>
        </div>
        ...
        <div className="border border-border bg-card p-6 md:p-10">
          {submitted ? (
            <div className="flex min-h-[500px] flex-col items-start justify-center">
              <p className="mb-5 text-xs uppercase tracking-[0.25em] text-muted-foreground">{data.success_subtitle}</p>
              <h2 className="max-w-md text-4xl font-light leading-tight tracking-[-0.04em]">{data.success_title}</h2>
